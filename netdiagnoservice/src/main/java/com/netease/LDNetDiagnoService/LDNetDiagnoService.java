...
public class LDNetDiagnoService extends
    ...
    public LDNetDiagnoService(Context context, String theAppCode,
                              ...
                              String theDeviceID, LDNetDiagnoListener theListener) {
        super();
    ...
    protected String doInBackground(String... params) {
        if (this.isCancelled())
            return null;
        return this.startNetDiagnosis(params);
    }
     ...
     * 开始诊断网络
     */
    public String startNetDiagnosis(String... hosts) {
        ...
        if (_isNetConnected) {
            ...
            // TCP三次握手时间测试
            recordStepInfo("\n开始TCP连接测试...");
            _netSocker = LDNetSocket.getInstance(_context);
            ...
            for (int i = 0; i < 1; i++) {
                ...
                for (String host : hosts) {
                    recordStepInfo("\n\ntraceroute: " + host + "\n");
                    _traceRouter.startTraceRoute(host);
                }
            }
    ...
    public void OnNetTraceUpdated(String log) {
        ...
        if (TextUtils.isEmpty(log) && logStr != null) {
            log = log.replaceAll("\\n", "");
        }
//        log = log.trim();
        if (log.matches("\\s*\\d+:.+") && logStr != null) {
            if (log.matches("\\s*1:.+")) {
                publishProgress(logStr + "\n");
            }
            logStr = null;
        }
        if (this._traceRouter != null && this._traceRouter.isCTrace) {
            ...
            if (TextUtils.isEmpty(log) || log.contains("ms") || log.contains("***")) {
                _logInfo.append(logStr);
                publishProgress(logStr);
                logStr = null;
            }
        } else {
